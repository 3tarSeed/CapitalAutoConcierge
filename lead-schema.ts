...
import {inServiceArea,serviceAreaLabel} from './service-area';
export const leadSchema=z.object({
 requestId:z.string().uuid(),year:z.string().regex(/^\d{4}$/).refine(v=>+v>=1950&&+v<=new Date().getFullYear()+1),
 make:z.string().trim().min(1).max(60),model:z.string().trim().min(1).max(100),trim:z.string().trim().max(100),zip:z.string().regex(/^\d{5}$/).refine(inServiceArea,{message:`We currently serve ${serviceAreaLabel}. Please enter a ZIP code in that area.`}),
