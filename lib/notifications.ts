...
type LeadNotification={
 ...
 timing:string;deadline?:string;selected:string[];estimate:{low:number;high:number;lines:EstimateLine[]};
};
...
async function sendEmail(lead:LeadNotification){
 ...
 const s=summary(lead),desk=e.CONCIERGE_DESK_URL||'https://capital-auto-concierge.adam-gouda.chatgpt.site/leads';
 const services=lead.estimate.lines.map(line=>`${clean(line.name)} — ${clean(line.package)}`).join('<br>')||lead.selected.map(clean).join('<br>');
